...
export function parseWellfoundJobs(html: string): JobListing[] {
...
export function parseWellfoundJobDetail(html: string): WellfoundJobDetail {
  const jsonLdJobs = parseJsonLdJobs(html);
  const jsonLdDescription = jsonLdJobs.find(
    (job) => job.description !== undefined,
  )?.description;
...
function parseWellfoundJobLink(
  $: cheerio.CheerioAPI,
  element: AnyNode,
...
function parseJsonLdJobs(html: string): JobListing[] {
  const $ = cheerio.load(html);
...
function parseJsonLdJob(value: unknown): JobListing | undefined {
...
function extractWellfoundDescription($: cheerio.CheerioAPI): string | undefined {
...
function extractWellfoundDescriptionFromPageText(
  ...
  const startMatch = /(About the role|About the job|Job description|Description)/i.exec(
    text,
  );
  const startIndex =
  ...
  const endMatch = /(Skills|Compensation|Benefits|Apply|Save|About the company)/i.exec(
...
function removeWellfoundDescriptionChrome(
  text: string | undefined,
): string | undefined {
  if (text === undefined) {
    return undefined;
  ...
  return normalizeText(
    text.replace(/^(About the role|About the job|Job description|Description)\s*/i, ""),
  );
}

function normalizeWellfoundUrl(value: string | undefined): string | undefined {
...
function extractWellfoundJobId(url: string): string | undefined {
  const pathMatch = /\/jobs\/([^/?#]+)/.exec(url);
  return pathMatch?.[1];
}

function normalizeJobTitle(value: string | undefined): string | undefined {
...
function getCompanyName(
  ...
  return normalizeText(
    container.find("h2, h3, [data-test='StartupName']").first().text(),
  );
}

function extractSalary(text: string): string | undefined {
  const match = /\$\d{2,3}k\s*[–-]\s*\$\d{2,3}k(?:\s*\/\s*yr)?|\$\d{2,3}k\+/i.exec(
    text,
  );
  return match?.[0];
}
...
function extractLocation(text: string): string | undefined {
  ...
  const locationMatch = /^\s*[•,]\s*([^•]+?)(?:\s*[•,]\s*\$|\s*[•,]\s*(?:today|yesterday|\d+\s+(?:day|week|month)s?\s+ago)|$)/i.exec(
    afterMode,
  );
...
function extractDatePosted(text: string): string | undefined {
...
function getJsonLdCompanyName(value: unknown): string | undefined {
  if (Array.isArray(value)) {
...
function getJsonLdLocation(value: unknown): string | undefined {
  if (Array.isArray(value)) {
...
function getJsonLdSalary(value: unknown): string | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
