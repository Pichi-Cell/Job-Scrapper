...
const ACCENTURE_SEARCH_API_URL =
  "https://www.accenture.com/api/accenture/jobsearch/result";
const ACCENTURE_BASE_URL = "https://www.accenture.com";
const ACCENTURE_REFERER =
  "https://www.accenture.com/ar-es/careers/jobsearch?jk=&sb=1&vw=0&is_rj=0&pg=1&sk=software%20engineering&ba=industry%20x";
...
export class AccentureScraper implements JobScraper {
  readonly source = "Accenture";
  ...
  async scrape(options: ScraperOptions = {}): Promise<ScraperResult> {
    ...
    if (!response.ok) {
      throw new Error(
        `Accenture search request failed with status ${response.status}`,
      );
    }
...
type AccenturePayload = Record<string, unknown> | AccentureJob[];
...
function parseAccentureResponse(text: string): AccenturePayload {
  if (text.trim() === "") {
    return [];
  }

  return JSON.parse(text.replace(/&#34;/g, '"')) as AccenturePayload;
}

function mapAccentureJobs(payload: AccenturePayload): JobListing[] {
  const jobs = Array.isArray(payload)
    ? payload
    : findFirstArray<AccentureJob>(payload, [
        "jobs",
        "jobResults",
        "results",
        "searchResults",
...
function mapAccentureJob(job: AccentureJob): JobListing | undefined {
  const title = job.title ?? job.jobTitle ?? job.jobName;
  const url = normalizeAccentureUrl(
    job.jobDetailUrl ?? job.jobUrl ?? job.url ?? job.applyUrl ?? job.internalReferUrl,
  );
...
function normalizeAccentureUrl(value: string | undefined): string | undefined {
  if (value === undefined) {
    return undefined;
  }
...
function normalizeAccentureLocation(job: AccentureJob): string | undefined {
...
function extractAccentureJobId(url: string): string | undefined {
  try {
...
function normalizeAccentureDate(value: string | number | undefined): string | undefined {
...
function normalizeAccentureRemote(value: string | undefined): boolean | undefined {
  if (value === undefined) {
    return undefined;
  }
