import {
  parseDynamiteJobs,
  type DynamiteSearchResponse,
} from "../parsers/dynamite.parser.js";
...
const DYNAMITE_ALGOLIA_URL =
  "https://49HKL9G3SB-dsn.algolia.net/1/indexes/prod_jobs/query";
...
export class DynamiteScraper implements JobScraper {
  readonly source = "Dynamite Jobs";
...
async function fetchDynamiteSearch(
  url: URL,
  options: ScraperOptions,
): Promise<DynamiteSearchResponse> {
  for (let attempt = 1; attempt <= REQUEST_RETRIES; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Dynamite Jobs search request failed with status ${response.status}`,
        );
      }

      return (await response.json()) as DynamiteSearchResponse;
    } catch (error) {
...
function buildDynamiteSearchParams(options: ScraperOptions): string {
...
function buildFacetFilters(options: ScraperOptions): unknown[] {
...
function buildSearchQuery(options: ScraperOptions): string {
...
function getCategoryFilter(category: string | undefined): string | undefined {
...
function getSkillFilters(skills: string | undefined): string[] {
...
function getLocationFilters(location: string | undefined): string[] {
