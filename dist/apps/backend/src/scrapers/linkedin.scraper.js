import { parseLinkedInJobDetail, parseLinkedInJobs, } from "../parsers/linkedin.parser.js";
...
function buildLinkedInSearchUrl(options, page) {
...
async function fetchLinkedInSearch(url) {
    for (let attempt = 1; attempt <= REQUEST_RETRIES; attempt += 1) {
        ...
        try {
            ...
            if (isBlockedStatus(response.status)) {
                throw new Error(`LinkedIn public jobs request was blocked with status ${response.status}; authentication, CAPTCHA, and challenge bypass are not supported`);
            }
            if (!response.ok) {
                throw new Error(`LinkedIn public jobs request failed with status ${response.status}`);
            }
...
async function enrichLinkedInJob(job) {
    try {
        ...
        console.warn(`[scraper:LinkedIn] Could not fetch job detail for ${job.url}: ${message}`);
        return job;
    }
}
function buildLinkedInDetailUrl(jobId) {
...
function isLinkedInPolicyError(error) {
