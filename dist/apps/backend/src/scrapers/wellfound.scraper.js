import { parseWellfoundJobDetail, parseWellfoundJobs, } from "../parsers/wellfound.parser.js";
...
function buildWellfoundSearchUrl(options, page) {
...
async function enrichWellfoundJobs(jobs) {
    const enrichedJobs = [];
...
async function enrichWellfoundJob(job) {
    try {
        ...
        console.warn(`[scraper:Wellfound] Could not fetch job detail for ${job.url}: ${message}`);
        return job;
    }
}
async function fetchWellfoundHtml(url) {
    for (let attempt = 1; attempt <= REQUEST_RETRIES; attempt += 1) {
        ...
        try {
            ...
            if (isBlockedStatus(response.status)) {
                throw new Error(`Wellfound public jobs request was blocked with status ${response.status}; authentication, CAPTCHA, and challenge bypass are not supported`);
            }
            if (!response.ok) {
                throw new Error(`Wellfound public jobs request failed with status ${response.status}`);
            }
...
function isWellfoundPolicyError(error) {
